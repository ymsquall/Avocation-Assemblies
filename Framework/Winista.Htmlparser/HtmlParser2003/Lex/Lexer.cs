				...
				else
				{
					switch (state)
					{
                        ...
                        default:
							...
							throw new System.Exception("state " + state);
#else
		...
		protected internal virtual INode ParseTag(int start)
		{
			...
			while (!done)
			{
				...
				switch (state)
				{
                    ...
                    default:
#if NETFX_CORE && UNITY_METRO && !UNITY_EDITOR
						throw new System.Exception("how the hell did we get in state " + state);
#else
...
#if NETFX_CORE && UNITY_METRO && !UNITY_EDITOR
					throw new System.Exception("jsp with no code!");
#else
		...
		public virtual INode ParseCDATA(bool quotesmart)
		{
			...
			while (!done)
			{
				...
				switch (state)
				{
                    ...
                    default:
#if NETFX_CORE && UNITY_METRO && !UNITY_EDITOR
						throw new System.Exception("how the fuck did we get in state " + state);
#else
