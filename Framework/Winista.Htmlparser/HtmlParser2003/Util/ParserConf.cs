...
namespace Winista.Text.HtmlParser.Util
{
	...
	public class ParserConf
	{
		...
		*/
		...
		{
            PlayerPrefs.SetString(name, value_Renamed.ToString());
		...
		public virtual int GetInt(System.String name, int defaultValue)
        {
            ...
            /*
            return s == null ? defaultValue : Props[name];
		...
		public virtual System.Type GetClass(System.String propertyName, System.Type defaultValue, System.Type xface)
		{
			try
			{
                ...
                if (theClass != null && !Type_WP_8_1.Type.IsAssignableFrom(xface, theClass))
#else
                if (theClass != null && !xface.IsAssignableFrom(theClass))
#endif
                {
#if NETFX_CORE && UNITY_METRO && !UNITY_EDITOR
					throw new System.Exception(theClass + " not " + xface.FullName);
#else
        ...
        {
#if NETFX_CORE && UNITY_METRO && !UNITY_EDITOR
            if (!Type_WP_8_1.Type.IsAssignableFrom(xface, theClass))
#else
		...
		private void LoadResource(System.Collections.Specialized.NameValueCollection properties, String name, bool quietFail)
		{
			try
			{
				...
				{
                    UnityEngine.Debug.LogError("bad conf file: top-level element not <htmlparser-conf>");
				...
				for (int i = 0; i < props.Count; i++)
				{
					...
					{
                        UnityEngine.Debug.LogError("bad conf file: element not <property>");
