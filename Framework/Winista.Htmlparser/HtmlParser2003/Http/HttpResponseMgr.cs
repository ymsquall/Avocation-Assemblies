...
namespace Winista.Text.HtmlParser.Http
{
	...
	internal sealed class HttpResponseMgr
	{
		...
		public HttpResponseMgr(string url):this(url.ToString(), url)
		{
		}
		...
		/// <param name="url"></param>
        public HttpResponseMgr(System.String orig, string url)
			: this(orig, url, false)
		{
		}
        ...
        public HttpResponseMgr(System.String orig, string url, bool bHeaderOnly)
		{
        ...
        /*
        internal void ProcessRequest(System.String orig, string url, bool bHeadersOnly)
		{
			//this.m_strOrig = orig;
            ...
            if (!"http".Equals(url))
			{
				throw new System.Exception("Not an HTTP url:" + url);
			}
			...
			try
			{
				...
				if(!bHeadersOnly)
				{
                    ...
                    if ((PlatformTools.CompareStr("gzip", contentEncoding, true) == 0) ||
                        (PlatformTools.CompareStr("x-gzip", contentEncoding) == 0))
					{
                    ...
                    else if (PlatformTools.CompareStr("deflate", contentEncoding, true) == 0)
					{
						System.IO.MemoryStream memStream = null;
						//
						// Need to decompress the response.
