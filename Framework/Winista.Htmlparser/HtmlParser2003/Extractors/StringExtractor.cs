...
namespace Winista.Text.HtmlParser.Extractors
{
	...
	public class StringExtractor : NodeVisitor
	{
		...
		protected internal virtual void CarriageReturn()
		{
            ...
            if ((0 != length) && ((Winista.Text.HtmlParser.Util.StringUtil.NEWLINE_SIZE <= length) && (!mBuffer.ToString(length - Winista.Text.HtmlParser.Util.StringUtil.NEWLINE_SIZE, Winista.Text.HtmlParser.Util.StringUtil.NEWLINE_SIZE).Equals(Winista.Text.HtmlParser.Util.StringUtil.NEWLINE))))
			{
                mBuffer.Append(Winista.Text.HtmlParser.Util.StringUtil.NEWLINE);
			}
		}
			...
			if (!mIsScript && !mIsStyle)
			{
				...
				if (!mIsPre)
				{
					...
					if (ReplaceNonBreakingSpaces)
						text = text.Replace('\u00a0', ' ');
					if (Collapse)
                        Winista.Text.HtmlParser.Util.StringUtil.CollapseString(mBuffer, text);
					else
						mBuffer.Append(text);
				}
				else
